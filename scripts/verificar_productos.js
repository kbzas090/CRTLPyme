...
async function verificarProductos() {
  ...
  try {
    ...
    const minimarketTenant = await prisma.tenant.findFirst({
      where: {
        OR: [
          ...
          { businessName: { contains: 'Don Luis', mode: 'insensitive' } }
        ]
      }
    });

    if (minimarketTenant) {
      console.log(`✓ Tenant encontrado: ${minimarketTenant.businessName} (ID: ${minimarketTenant.id})`);
      console.log('');

      const totalInventoryCount = await prisma.tenantInventory.count({
        where: { tenantId: minimarketTenant.id }
      });

      console.log(`✓ Total items en inventario de ${minimarketTenant.businessName}: ${totalInventoryCount}`);
      console.log('');
      ...
      if (minimarketInventory.length > 0) {
        console.log('Primeros 10 productos en inventario:');
        console.log('');
        minimarketInventory.forEach((item, index) => {
          console.log(`${index + 1}. ${item.masterProduct.name} (SKU: ${item.masterProduct.sku})`);
          console.log(`   - Stock: ${item.stock}, Precio: $${item.masterProduct.suggestedPrice}, Categoría: ${item.masterProduct.category}`);
        });
      }
    } else {
    ...
    tenants.forEach(tenant => {
      console.log(`- ${tenant.businessName}: ${tenant._count.tenantInventories} productos`);
    });
    console.log('');
